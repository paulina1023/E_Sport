...
export function validarMotivoEliminacion(req, res, next) {
  if (!["Descalificado", "Sancionado"].includes(req.body.motivo)) {
    return res.status(400).json({
      error: "El motivo debe ser Descalificado o Sancionado",
    });
  }
  return next();
}
